...
# Controls how many stocks are queued for fetching at the same moment. This
# does NOT control the actual request rate to Yahoo -- that's enforced by
# yf_ratelimit.py's shared _throttle() gate (one request every MIN_DELAY_S,
# globally, no matter how many workers are queued here -- empirically
# verified concurrency-safe under 16 simultaneous workers, see 2026-09-03
# fix notes in yf_ratelimit.py). This just bounds how many threads are
# alive/queued waiting their turn. Lowered from 6 to 3 as extra margin.
_YF_SEMAPHORE_COUNT = 3
...
st.sidebar.info("""
**⚡ Fixed 2026-09-03: the shared throttle was silently doing nothing**  
`yf_ratelimit.py`'s pacing gate was broken (see file header) — every request fired with
zero delay regardless of this setting. Now fixed and stress-tested under 16 concurrent
workers with zero spacing violations. Worker count below only affects how many stocks
are queued at once, not how fast Yahoo actually gets hit — that's fixed at one request
every MIN_DELAY_S no matter what you set here.
""")

max_workers_ui = st.sidebar.slider(
    "Parallel workers",
    min_value=1, max_value=16, value=3, step=1,
    help="How many stocks to fetch simultaneously. Every worker still goes through the same "
         "shared throttle in yf_ratelimit.py, so this controls how many are QUEUED at once, "
         "not how fast requests actually reach Yahoo (that's fixed at one every "
         "MIN_DELAY_S regardless of this number, and empirically verified concurrency-safe). "
         "Lower workers just means less time spent queued per stock, not fewer 429s. "
         "Default lowered from 6 to 3 as extra margin after repeated 429s in production.",
    key="sp_max_workers"
)
...
if _do_scan:
    ...
    if _resume_scan:
        ...
        # Symbols that FAILED last run are retried on resume, not excluded
        # forever -- only successful/filtered symbols count as "done". Before
        # this fix, already_scanned included failed symbols too (see the live
        # loop below), so a symbol that failed once (rate-limit, transient
        # network error, anything) would silently never be attempted again on
        # any future resume unless the user noticed and clicked START FRESH
        # for a full rescan. failed_symbols is reset here since every one of
        # them is about to get a fresh attempt this run, not carried forward
    ...
    try:
        with ThreadPoolExecutor(max_workers=_max_workers) as executor:
            ...
            for future in as_completed(future_to_sym):
                ...
                if status == 'ok':
                    already_scanned.add(symbol)  # genuinely done -- excluded from any future resume
                    results.append(analysis)
                elif status == 'filtered':
                    already_scanned.add(symbol)  # genuinely done (didn't meet criteria, but WAS scanned)
                    filtered_out += 1
                else:
                    # 'failed' deliberately NOT added to already_scanned -- see the
                    # resume-path comment above for why. Still counted toward
                    # `completed`/progress for THIS run, and still recorded in
                    # failed_symbols so it's visible in this run's stats -- just
                    # not permanently excluded from a future resume.
                    failed += 1
